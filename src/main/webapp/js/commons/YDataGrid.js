...
var YDataGrid = function(config){
		...
		var Handler = {
			...
			edit:  function(callback){
				...
				if (Utils.checkSelectOne(record)){
					...
					XHY.getById(Action.getId,data,function(result){
						XHY.closeProgress();
				...
				if (Utils.checkSelect(records)){
					$.messager.confirm('确认','您确定要删除这条记录?',function(r){  
					    if (r){
					    	...
					    	var data = arr.join("&");
					   		XHY.deleteForm(Action.remove,data,function(result){
								XHY.closeProgress();
								Events.refresh();
								//回调函数
								if(jQuery.isFunction(callback)){
			...
			save: function(callback){
				if(Form.edit.form('validate')){
					...
					XHY.saveForm(Form.edit,function(data){				
						XHY.closeProgress();
		...
		var initGrid = function(){
			var dataconfig = {
				...
				collapsible:false,
				remoteSort: false,
				sortName:dataGrid.sortName , //排序字段
				sortOrder:dataGrid.sortOrder || 'asc',//排序顺序
				pagination:dataGrid.pagination==false?false:true, //是否显示分页栏
				rownumbers:true,
				singleSelect:true,
				checkOnSelect:false,
				selectOnCheck:false,
				...
				frozenColumns:dataGrid.frozenColumns||[[]],//添加了冻结列项
				columns: dataGrid.columns,
				toolbar: _toolbar,
				onLoadSuccess: dataGrid.onLoadSuccess || function(){
					Grid.datagrid('unselectAll');
					Grid.datagrid('uncheckAll');
					Grid.datagrid('doCellTip',{delay:300});   
				},
				...
				onClickCell:dataGrid.onClickCell || function(rowIndex, field, value){
				},
				onDblClickCell:dataGrid.onDblClickCell || function(index, field, value){
				}
			};
		...
		var initTbar = function(){
			...
			//查询页面授权的btnType
			XHY.ajaxJson(_url,data,function(data){
				if(data.success){
					if(data.allType){
...
//							Grid.datagrid({'toolbar':newBars});
		...
		var initForm = function(){
			if(Form.search && Form.search[0]){
				...
				Form.search.find("#btn-clearsearch").click(function() {
					$('input').attr("value","");
					return false;
				});
			}
		...
		this.init = function(){
			...
			initForm();
			initWin();
//			initGrid();//初始化dataGrid，因为异步问题 现在调入initTbar()执行完毕后执行
		};
		//调用初始化
		return this;
};
$.extend($.fn.datagrid.methods,{
	 ysbreload: function(jq,param){
	  return jq.each(function(){
	   //显示第一页数据
	   $(this).datagrid("options").pageNumber = 1;
	   //分页栏上跳转到第一页
	   $(this).datagrid('getPager').pagination({pageNumber: 1});
	   $(this).datagrid('reload',param);
	  });
	 }
